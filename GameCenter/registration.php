...
<body>
	<form class="login_register" action="registration.php" method="post">
		...
		include 'php/functions.php';
			if(isset($_POST['submit']) ){
				...
				if($password == $retype_password) {
					...
					$data = mysqli_fetch_row($result_sql);

					if( empty($data) ){
						$template = "/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{6,20}$/";

						if( preg_match($template, $password) ) {
							$query = "INSERT INTO user_data(username, password, first_name, last_name, sex, email, birthday, id, is_active)
									  VALUES('$user_name', '$password', '$fname', '$lname', '$sex', '$email', '$birthday', '', '1')";

							(mysqli_query($connection, $query) == true) ? print show_message_result("Data insert complete!") :
															  			  print show_message_result("Error: ".mysqli_error($connection) );
					...
					show_message_result('Passwords don\'t match!');
